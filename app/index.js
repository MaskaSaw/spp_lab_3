...
const page401 = "/auth.html";
const page404 = "/404.html";
const page500 = "/500.html";
const homePage = "/logic.html";
...
app.post("/auth", async function(request, response) { 
    ...
    let result = await new Promise((resolve, reject) => {
        fs.readFile(__dirname + '/userData/users', "utf8", function read(err, data) {
            if (err) {
                resolve(_SMTH_BAD_);
    ...
    let regex = new RegExp("_" + request.body.login + "\\$ : \\$" + request.body.password + "_");

    if (regex.test(allUsersData)) {
        let secret;
        result = await new Promise((resolve, reject) => {
            fs.readFile(__dirname + '/token/secret', "utf8", function read(err, secretData) {
                if (err) {
                    resolve(_SMTH_BAD_);
                    return response.status(500).sendFile(__dirname + page500);
                }
